...
"""
# extracting out the quant files
import glob
from shutil import copy2

for full_name in glob.glob('chrisRNA/*/quant.sf'):
    trim = (full_name.split('/'))
    output_name = ('chrisRNA/quants/' + trim[1] + '.sf')
    copy2(full_name, output_name)
"""


# subsetting the read samples table
def subset_read_table(cont, cond):
    read_table = pd.read_csv('lists/all.txt', index_col = 0, delimiter = '\t')
    subset = read_table[(read_table['Condition'] == control) | (read_table['Condition'] == condition)]
    subset.to_csv('lists/' + condition + '_to_' + control + '.txt', sep = '\t', index = True)


baseScript = """
# R script for DESeq2 processing of {} and {}
...
write.csv(as.data.frame(res), file=("out/processed/Condition_{}_vs_{}.csv"))
...
for sequence in range(1,2):
    condition = 'NIN-KO_induced'
    control = 'WT_induced'
    with open('scripts/allSamples_' + condition + '_to_' + control + '.r', 'w') as output:
        subset_read_table(condition, control)
        output.write(baseScript.format(control, condition, condition, control, control, condition, control, condition, control))
